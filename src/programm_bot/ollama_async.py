...
async def ollama_chat(messages: list[dict[str, str]], model: str = OLLAMA_MODEL) -> str:
    ...
    Args:
        messages (list[dict[str, str]]): Список словарей с сообщениями.
            Каждый словарь должен содержать ключи 'role' (например, 'user' или 'system')
            и 'content' с текстом запроса.
        model (str, optional): Имя используемой модели. По умолчанию `qwen2.5-coder:0.5b`.
        
    Returns:
        str: Сгенерированный моделью текст (в данном контексте — конвертированный код).
        
    Raises:
        httpx.HTTPStatusError: Если сервер Ollama вернул код ошибки (например, 404 или 500).
